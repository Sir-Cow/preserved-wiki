---
hide:
  - footer

search:
  exclude: true
---

# 🌊 Torrential

---

**This mod adds much needed depth into Minecraft's fishing system by adding fishing rod parts, fishing loot table changes, and an improved conduit.**

## ⚓ Fishing Rod Parts

---

Obtain your first fishing rod parts by getting some aquatic fiber, which can be found in the fishing loot pool or in a buried treasure chest. By using some common tool materials, you can turn the fiber into a hook, line, or sinker!

- Fishing hooks will increase your rod's fishing speed allowing you to catch fish at a more efficient rate.
- Fishing lines will increase your rod's fortune allowing you to collect multiple non-treasure pulls.
- Sinkers will increase your rod's luck allowing you to find more treasure drops, more often.

Craft an angling table to apply and remove your new parts. View the durability of your parts in the new tooltip added onto your fishing rod!

## 📦 Caches

---

Don't want to rush the End for shulker boxes? Try your luck at catching a cache! While filled with treasure, caches also work as a weaker version of the sought after shulker box.

## 🧿 Updated Conduit

---

The conduit is now a powerful tool for fishing as it grants the player bonus fishing speed and luck to those nearby. As you power up your conduit with additional prismarine blocks, the effect of the conduit will strengthen.

Conduit power will also grant the player with:

- Enhanced mining speed. (stacks with Haste)
- Riptide usage outside of the water.
- Unlimited water breathing.
- Night vision while underwater.

## 🐟 Notable Changes

---

- Fishing rods no longer have durability. No need to maintain your rod, just make sure your fishing rod parts are well kept.
- Fishing rods cannot have enchantments applied to them.
- Fisherman villagers can now trade fishing rod parts to the player.
- Luck potions can now be brewed using rabbit hide. Strengthen it with glowstone dust or extend it using redstone.
- New advancements! Learning the mod's content couldn't be easier.

## ⚙️ Modpack Usage

---

Torrential is free to be used in any custom mod packs as long as credit is provided. Compatibility with other mods has not been tested for.

## 🐛 Issues

---

If you happen to experience any issues while using this mod, please report it to our [GitHub issue tracker](https://github.com/Sir-Cow/torrential/issues)! Please be as clear as possible when reporting any bugs or issues.

## 🔥 Infernal Origins

---

The contents of this mod originate from our parent mod [Preserved: Inferno](https://modrinth.com/mod/preserved-inferno). Please consider checking it out if you enjoy this content!

## 🖥️ Download

---

```embed
url:            https://modrinth.com/mod/torrential
name:           Torrential on Modrinth
desc:           Official Torrential mod page for Modrinth.
image:          https://sir-cow.github.io/preserved-wiki/img/torrential/torrential.png
favicon:        https://cdn.jsdelivr.net/gh/selfhst/icons/svg/modrinth.svg
favicon_size:   25
target:         new
accent:         4C59BFE0
```

```embed
url:            https://www.curseforge.com/minecraft/mc-mods/torrential
name:           Torrential on Curseforge
desc:           Official Torrential mod page for Curseforge.
image:          https://sir-cow.github.io/preserved-wiki/img/torrential/torrential.png
favicon:        https://cdn.simpleicons.org/curseforge/F16436
favicon_size:   25
target:         new
accent:         4C59BFE0
```
